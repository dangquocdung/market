...
class OrdersController extends Controller
{
    ...
    function view(){
        $orders = DB::table('orders')->where("send", "1")->orderBy('updated_at', 'desc')->get();
        ...
        $ordersdetails = null;
        if ($orders->count() != 0)
            $ordersdetails = DB::table('ordersdetails')->where('order', '=', $orders[0]->id)->get();
        ...
        return view('orders', ['iorders' => $orders, 'iusers' => $iusers, 'iorderstatus' => $orderstatus,
            'irestaurants' => $restaurants, 'idrivers' => $drivers, 'currency' => $currency, 'coupons' => $coupons,
            'iordersdetails' => $ordersdetails, 'texton' => "", 'text' => '', 'rightSymbol' => $rightSymbol, 'symbolDigits' => $symbolDigits]);
    }
